...
class _GestionMensuelLiveState extends State<GestionMensuelLive> with SingleTickerProviderStateMixin {
  ...
  void selectedIcon(int index) {
    setState(() {
      ...
      icones = IconData(GlobalConstant.icons[index], fontFamily: 'MaterialIcons');
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    double montantChargeDouble = CalculMontantMensuel().montantCharge(_listMontantUniverselle);
    double montantChargeLiveDouble = CalculMontantMensuel().montantChargeLive(_listMontantUniverselleLive);
    double montantRevenuDouble = CalculMontantMensuel().montantRevenu(_listMontantUniverselle);
    double montantRevenuLiveDouble = CalculMontantMensuel().montantRevenuLive(_listMontantUniverselleLive);
    double montantTotalsDouble = CalculMontantMensuel().montantTotals(_listMontantUniverselle);
    double montantTotalsLiveDouble = CalculMontantMensuel().montantTotalsLive(_listMontantUniverselleLive);
                    ...
                    children: [
                          ...
                          DropdownMenuItem(
                            ...
                            child: Row(
                              children: [
                                ...
                                SizedBox(width: 10),
                                Text("depensePonctuelle", style: TextStyle(color: Colors.red)),
                              ],
                            ),
                          ),
                          DropdownMenuItem(
                            ...
                            child: Row(
                              children: [
                                ...
                                SizedBox(width: 10),
                                Text("RevenuPonctuel", style: TextStyle(color: Colors.green)),
                              ],
                            ),
                          ),
                        ],
                      ...
                      Accordion(
                        ...
                        children: [
                          AccordionSection(
                            isOpen: false,
                            header: Text("Choisir une icone", style: TextStyle(color: Colors.white, fontSize: 17)),
                            content: gridIcon(),
                          ),
                        ],
                      ),
                      ...
                      Center(
                        child: IconButton(
                          ...
                          icon: Icon(Icons.check_circle, color: Colors.orange[900]),
                          onPressed: () {
                            if (formKey.currentState!.validate()) {
                              formKey.currentState!.save();

                              if (_checkbox) {
                                Provider.of<EasyController>(context, listen: false)
